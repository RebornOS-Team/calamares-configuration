...
- [ ] Create and push a tag, for example `git tag -s -a v0.1.60 -m "This is version 0.1.60" && git push origin v0.1.60`
- [ ] If there was a mistake and if you want to yank the release, run something like `git tag -d v0.1.60 && git push --delete origin v0.1.60`. Then the errors can be fixed and the previous step can be repeated to create a new tag.